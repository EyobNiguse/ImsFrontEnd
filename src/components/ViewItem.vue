          ...
          <fieldset class="view-items-container">
            ...
            <table class="view-items">
              ...
              >
                <fieldset>
                  <legend>
                    <h3>Search Inventory History</h3>
                  </legend>
                  <table>
                    <tr>
                      <td>From</td>
                ...
                <table class="view-items">
                  ...
                  <tr :key="x.ItemId" v-for="x in displayedHistory">
                    ...
                    <td>{{ x.GRNNO || "-" }}</td>
                    <td>{{ x.IN || "-" }}</td>
                    <td>{{ x["Purchase-Price"] || "-" }}</td>
                    <td>{{ x["Purchase-Total"] || "-" }}</td>
                    <td>{{ x.REFNO || "-" }}</td>
                    <td>{{ x.OUT || "-" }}</td>
                    <td>{{ x["Sales-Price"] || "-" }}</td>
                    <td>{{ x["Sales-Total"] || "-" }}</td>
                    <td>{{ x["Balance"] || "-" }}</td>
                    <td>{{ x["FIFO-ITEM-GRN"] || "-" }}</td>
                    <td>{{ x["FIFO-COGS"] || "-" }}</td>
                    <td>{{ x["Average-COGS"] || "-" }}</td>
                    <td>{{ x["Average-Purchase-Price"] || "-" }}</td>
                  </tr>
                </table>
                <span>
                  <span class="prev">
                  ...
                  <span class="number">
                    <button
                      class="btn-submit-page"
                      :key="pageNumber"
                      v-for="pageNumber in pagesList.slice(
                        pageList - 1,
                        pageList + 5
                      )"
                      @click="pageList = pageNumber"
                    >
                      {{ pageNumber }}
                    </button>
                  </span>
                  <span class="next">
                    <button
                      @click="pageList++"
                      v-if="pageList < pagesList.length"
                      class="btn-submit"
                    >
                      Next
                    </button>
              ...
              <tr
                :name="x.ItemID"
                v-bind:key="index"
                v-for="(x, index) in displayedItems"
              >
